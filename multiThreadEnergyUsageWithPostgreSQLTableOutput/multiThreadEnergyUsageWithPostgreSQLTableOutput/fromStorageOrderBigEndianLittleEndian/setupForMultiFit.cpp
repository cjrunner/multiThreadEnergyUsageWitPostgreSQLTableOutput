...
SetupForMultiFit::~SetupForMultiFit() {  //Destructor
    if (this->areWeInRunMode) { //When this is true, we're in test mode and there's nothing to free
...
SetupForMultiFit::SetupForMultiFit(bool b) { 
    this->areWeInRunMode = b; //Indicate we're in test mode.
    std::cout << "Here we are in constructor SetupForMultiFit::SetupForMultiFit(void) which does nothing. The only reason this constructor was called was becaue TblPolyFit's constructor was activated. The class TblPolyFit is a child of the class SetupForMultiFit and can inherit from SetupForMultiFit." << std::endl;
}
